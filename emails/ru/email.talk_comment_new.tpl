						...
						<table width="100%">
							<tr>
								<td style="vertical-align: top; width: 90px;">
									<div style="text-align: center;"><img src="{$oUserFrom->getAvatarUrl(100)}" style="
									width: 60px;
									border-radius: 50%;
									border: 1px solid #DFDFDF;
									padding: 2px;"/>			
									<br><strong>
										    <a href="{$oUserFrom->getProfileUrl()}">
											    {if $oUserFrom->getProfileName()}{$oUserFrom->getProfileName()}{else}{$oUserFrom->getDisplayName()}{/if}
											</a>
										</strong>
									</div>
								</td>
								<td style="vertical-align: top; text-align: justify; padding: 10px 15px 0px 10px;">{$oTalkComment->getText()|strip_tags}</td>
							</tr>
						</table>													
					{/if}
					
					<br>
					<a style="color: #6e6b92; text-decoration: none;" href="{router page='talk'}read/{$oTalk->getId()}/#comment{$oTalkComment->getId()}">Перейти к диалогу...</a>	
				</td>
			</tr>
